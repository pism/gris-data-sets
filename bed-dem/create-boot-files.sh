...
infile=Greenland_150m_mcb_jpl_v${ver}.nc
...
mv ${ibcaofile}_tif/${ibcaofile}.tif ${ibcaofile}.tif 
...
# rsync -rvu --progress  $user@beauregard.gi.alaska.edu:/data/tmp/data_sets/mc_beds/${infile} .
ibcao=IBCAO_V3_500m_RR_tif/IBCAO_V3_500m_RR.tif
#for GRID in 36000 18000 9000 4500 3600 1800 1500 1200 900 600 450; do
 for GRID in 3600; do
    outfile=pism_Greenland_${GRID}m_mcb_jpl_v${ver}.nc
    echo "creating $GRID m grid file"
    gdalwarp -overwrite -r average  -t_srs EPSG:3413 -te $xmin $ymin $xmax $ymax -tr $GRID $GRID -of GTiff ${ibcaofile}.tif ${ibcaofile}_epsg3413_g${GRID}m.tif
    gdalwarp -overwrite -of netCDF -s_srs EPSG:3413 -t_srs EPSG:3413  ${ibcaofile}_epsg3413_g${GRID}m.tif  ${ibcaofile}_epsg3413_g${GRID}m.nc
    ncrename -v Band1,bathymetry ${ibcaofile}_epsg3413_g${GRID}m.nc
    ...
    for var in "errbed" "surface" "mask" "source" "thickness"; do
        ncks -A -v $var g${GRID}m_${var}_v${ver}.nc $outfile
        ncatted -a grid_mapping,$var,o,c,"polar_stereographic" $outfile
    done
    ncks -A -v bathymetry ${ibcaofile}_epsg3413_g${GRID}m.nc $outfile
    ncatted -a grid_mapping,bathymetry,o,c,"polar_stereographic" $outfile
    ...
    ncap2 -O -s "where(thickness<0) thickness=0; ftt_mask[\$y,\$x]=1b; where(mask==0) {thickness=0.; surface=0.;}; where(mask==0 || mask==3) {bed=bathymetry;};" $outfile $outfile
    ncks -O -v bathymetry -x $outfile $outfile
    # scp $outfile $user@beauregard.gi.alaska.edu:/data/tmp/data_sets/mc_beds/
done
